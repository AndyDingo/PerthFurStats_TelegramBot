...
#pragma warning disable RECS0022 // A catch clause that catches System.Exception and has an empty body
    ...
    class Program
    {
        ...
        /// <summary>
        /// Process our (most basic) slash commands only
        /// </summary>
        /// <param name="bot">The bot API.</param>
        /// <param name="update">The update</param>
        /// <param name="me">The user, or bot.</param>
        /// <param name="dt">The date/time component.</param>
        /// <remarks>Designed to work if regular commands are disabled.</remarks>
        private static async void nwProcessBasicSlashCommands(Api bot, Update update, User me, DateTime dt)
        {
            // Process request
            try
            {
                ...
                {
                    // Log to console
                    Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] " + update.Message.Chat.Id + " < " + update.Message.From.Username + " - " + text);
                    ...
                    switch (command.ToLowerInvariant())
                    {
                        ...
                        case "/alive":
                            bot.SendChatAction(update.Message.Chat.Id, ChatAction.Typing);
                            replyText = "Hi " + update.Message.From.FirstName + ", I am indeed alive, though in reduced capacity.";
                            break;
                        case "/toggledebug":
                        case "/debug":
                            ...
                            if (nwGrabString("debugMode") == "false")
                            {
                                ...
                                Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] * System: DEBUG MODE ENABLED!");
                            }
                            else
                            {
                                nwSetString("debugMode", "false");
                                Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] * System: DEBUG MODE DISABLED!");
                            }
                            break;
                        case "/version":
                        ...
                        case "/info":
                            ...
                            int aboutuse = nwGrabInt("cusage/about");
                            aboutuse++;

                            nwSetString("cusage/about", Convert.ToString(aboutuse));
                            break;
                        default:
                            replyText = "The command '" + update.Message.Text + "' was either not found in my command database, or is disabled in this mode.";
                            Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] * System: " + replyText);
                            break;
                    }
                    ...
                    {
                        Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] <" + me.FirstName + "> " + update.Message.Chat.Id + " > " + replyText);
                    ...
                    {
                        Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] <" + me.FirstName + "> " + update.Message.Chat.Id + " > " + replyText2);
                    ...
                    {
                        Console.WriteLine("[" + dt.ToString(nwParseFormat(true)) + "] <" + me.FirstName + "> " + update.Message.Chat.Id + " > " + replyTextMarkdown);
